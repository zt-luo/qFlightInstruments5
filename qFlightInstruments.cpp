...
void QADI::resizeEvent(QResizeEvent *)
{
...
void QADI::paintEvent(QPaintEvent *)
{
    ...
    // draw background
    {
        ...
        y_min = static_cast<int>(m_size/2*-40.0/45.0);
        y_max = static_cast<int>(m_size/2* 40.0/45.0);

        int y = static_cast<int>(m_size/2*pitch_tem/45.0);
        if( y < y_min ) y = y_min;
        if( y > y_max ) y = y_max;
        ...
        painter.drawChord(-m_size/2, -m_size/2, m_size, m_size,
                          static_cast<int>(gr*16), static_cast<int>((180-2*gr)*16));
    ...
    // draw pitch lines & marker
    {
        ...
        for(int i=-9; i<=9; i++) {
            ...
            if( i == 0 ) {
                ...
                l = static_cast<int>(l * 1.8);
            } else {
                painter.setPen(pitchPen);
            }

            y = static_cast<int>(m_size/2*p/45.0 - m_size/2*pitch_tem/45.0);
            x = l;
...
void QCompass::resizeEvent(QResizeEvent *)
{
    m_size = qMin(width(),height()) - 2*m_offset;
}
